...
async function waitForPortFree(port, timeoutMs = 8000, interval = 500) {
    ...
    while (Date.now() - start < timeoutMs && attempts < MAX_KILL_ATTEMPTS) {
        ...
        // Do not perform broad name-based kills here; rely on PID-based kills above
...
function killPidTree(pid) {
    ...
    try {
        if (process.platform === 'win32') {
            ...
            try {
                execSync(`wmic process where "ParentProcessId=${pid}" delete`, {
                    stdio: ['ignore', 'ignore', 'ignore'],
                    timeout: 2000
                });
            } catch (e) { }
...
async function killAllOnPort(port) {
    ...
    while (attempts < MAX_KILL_ATTEMPTS) {
        const currentPids = getPidsListeningOnPort(port);
        if (currentPids.length === 0) return Array.from(killedPids);
        ...
        // Rely on PID kills rather than name-based kills to avoid affecting other services
    ...
    return Array.from(killedPids);
}

async function waitForPortBound(port, timeoutMs = 10000, interval = 500) {
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
        const p = getPidsListeningOnPort(port);
...
async function startFrontendForce() {
    ...
    frontendProc.stdout.on('data', (data) => {
        ...
        if (output && !output.includes('Local:')) {
...
async function stopSpawned(procName) {
    ...
    const backendProcessNames = ['node.exe', 'node', 'server.js'];
    const frontendProcessNames = ['node.exe', 'node', 'vite', 'npm.exe', 'npm'];
    const processNames = procName === 'backend' ? backendProcessNames : frontendProcessNames;
...
async function renderStatus() {
    ...
    if (backPid) {
        backStats = await getStatsForPid(backPid);
    } else if (externalBackPids.length) {
        backStats = await aggregateStatsForPids(externalBackPids);
    }

    if (frontPid) {
        frontStats = await getStatsForPid(frontPid);
    } else if (externalFrontPids.length) {
        frontStats = await aggregateStatsForPids(externalFrontPids);
    }

    if (backPid) {
        console.log(`[Backend] 🟢 ativo  PID:${backPid}`);
    } else if (externalBackPids.length) {
        console.log(`[Backend] 🟠 ativo (externo)  PIDs:${externalBackPids.join(', ')}`);
    } else {
        console.log(`[Backend] 🔴 parado  PID:-`);
    }
